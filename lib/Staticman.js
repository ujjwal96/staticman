...
Staticman.prototype._checkAuth = function () {
  if (this.siteConfig.get('staticmanToken')) {
    if (this.options['staticman-token'] !== this.siteConfig.get('staticmanToken')) {
      return Promise.reject(errorHandler('INVALID_STATICMAN_TOKEN'))
    }
  }
...
Staticman.prototype._generateReviewBody = function (fields) {
  ...
  let message = this.siteConfig.get('pullRequestBody')
  if (this.siteConfig.get('pullRequestTable')) {
    message += markdownTable(table)
  }
